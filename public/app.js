        ...
        <div class="animate-in fade-in pt-4 pb-20 space-y-6">
            ...
            <div class="bg-white/[0.03] p-5 rounded-[20px] border border-white/10">
                ...
                <div class="space-y-3 text-xs">
                    <div class="flex justify-between border-b border-white/5 pb-2"><span class="text-gray-500">Username</span><span class="text-white font-semibold">${userData.username}</span></div>
                    <div class="flex justify-between pb-1"><span class="text-gray-500">Security</span><span class="text-[#10B981] font-semibold flex items-center gap-1">✓ Verified</span></div>
                </div>
            </div>
            <button onclick="localStorage.removeItem('token'); location.reload();" class="w-full bg-red-500/10 border border-red-500/30 text-red-400 py-3.5 rounded-[14px] font-bold text-xs hover:bg-red-500/20 transition-all">Sign Out</button>
        </div>
    `;
}
...
function render() {
    try {
        ...
        else if (currentTab === 'invite') content = `<div class="pt-20 text-center"><h2 class="text-2xl font-bold text-white">Referral Hub</h2><p class="text-xs text-gray-500 mt-2">Earn 15% on network deposits.</p></div>`;
        else if (currentTab === 'track') content = `<div class="pt-32 text-center text-xs text-gray-500">No active portfolios tracking.</div>`;
