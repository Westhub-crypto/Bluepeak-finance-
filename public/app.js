...
let resetStep = 1; 
let kycStep = 1; // State for KYC flow
...
async function switchTab(tab) {
    if (!isLoggedIn && tab !== 'login' && tab !== 'register' && tab !== 'forgot') {
...
// --------------------------------------------------------
// THE NEW ELITE KYC VERIFICATION PAGE
// --------------------------------------------------------
function KYCPage() {
    let content = '';
    ...
    if (activePlans.length === 0) {
        return `
            <div class="flex flex-col items-center justify-center py-32 text-center animate-in zoom-in duration-500">
                ...
                <h2 class="text-xl font-bold text-white mb-2 tracking-tight">Awaiting Portfolios</h2>
                <p class="text-[11px] text-gray-500 px-8 leading-relaxed">Our quantitative analysts are structuring new high-yield funds. Please check back shortly.</p>
            </div>
        `;
    }
